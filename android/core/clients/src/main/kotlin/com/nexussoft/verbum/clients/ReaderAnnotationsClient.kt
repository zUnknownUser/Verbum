...
class PreferenceReaderAnnotationsClient(private val preferences: PreferencesClient) : ReaderAnnotationsClient {
    @Serializable private data class Entry(val book: String,val chapter: Int,val verse: Int,val color: String?=null,val note: String="") {
        fun model()=ReaderAnnotation(PassageReference(book,chapter,verse..verse),color?.let { runCatching { HighlightColor.valueOf(it) }.getOrNull() },note)
    }
    ...
    override suspend fun save(annotation: ReaderAnnotation) = synchronized(lock) {
        ...
        preferences.setString("readerAnnotations",Json.encodeToString(values.map { Entry(it.reference.bookId,it.reference.chapter,it.reference.verses?.first ?: 1,it.highlight?.name,it.note) }))
    }
}
