import Models

/// bible.helloao.org addresses books by USFM code (`JHN`, `1SA`, `PSA`);
/// the app uses OSIS ids (`John`, `1Sam`, `Ps`). Both directions, all 66.
enum HelloAOBooks {
    static let usfmByOSIS: [BookID: String] = [
        "Gen": "GEN", "Exod": "EXO", "Lev": "LEV", "Num": "NUM", "Deut": "DEU", "Josh": "JOS", "Judg": "JDG", "Ruth": "RUT",
        "1Sam": "1SA", "2Sam": "2SA", "1Kgs": "1KI", "2Kgs": "2KI", "1Chr": "1CH", "2Chr": "2CH", "Ezra": "EZR", "Neh": "NEH",
        "Esth": "EST", "Job": "JOB", "Ps": "PSA", "Prov": "PRO", "Eccl": "ECC", "Song": "SNG", "Isa": "ISA", "Jer": "JER",
        "Lam": "LAM", "Ezek": "EZK", "Dan": "DAN", "Hos": "HOS", "Joel": "JOL", "Amos": "AMO", "Obad": "OBA", "Jonah": "JON",
        "Mic": "MIC", "Nah": "NAM", "Hab": "HAB", "Zeph": "ZEP", "Hag": "HAG", "Zech": "ZEC", "Mal": "MAL",
        "Matt": "MAT", "Mark": "MRK", "Luke": "LUK", "John": "JHN", "Acts": "ACT", "Rom": "ROM", "1Cor": "1CO", "2Cor": "2CO",
        "Gal": "GAL", "Eph": "EPH", "Phil": "PHP", "Col": "COL", "1Thess": "1TH", "2Thess": "2TH", "1Tim": "1TI", "2Tim": "2TI",
        "Titus": "TIT", "Phlm": "PHM", "Heb": "HEB", "Jas": "JAS", "1Pet": "1PE", "2Pet": "2PE", "1John": "1JN", "2John": "2JN",
        "3John": "3JN", "Jude": "JUD", "Rev": "REV",
    ]

    static let osisByUSFM: [String: BookID] = Dictionary(uniqueKeysWithValues: usfmByOSIS.map { ($1, $0) })
}

/// Which helloao translation to read, by device language. All open-licensed:
/// BSB (Berean Standard Bible, English) and Bíblia Livre (Brazilian Portuguese).
public enum HelloAOTranslation {
    public static func id(for language: BookLanguage) -> String {
        switch language {
        case .english: "BSB"
        case .portuguese: "por_blj"
        }
    }

    public static func name(for id: String) -> String {
        switch id {
        case "BSB": "Berean Standard Bible"
        case "por_blj": "Bíblia Livre"
        case "web": "World English Bible"
        default: id
        }
    }
}
