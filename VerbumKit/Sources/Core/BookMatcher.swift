...
public enum BookMatcher {
    ...
    public static func books(matching query: String) -> [BibleBook] {
        let key = normalize(query)
        guard !key.isEmpty else { return [] }
        var exact: [BibleBook] = []
        var prefix: [BibleBook] = []
        for book in BibleBook.canon {
            let keys = [book.name] + book.abbreviations
            if keys.contains(where: { normalize($0) == key }) {
                exact.append(book)
            } else if keys.contains(where: { normalize($0).hasPrefix(key) })
                || book.name.split(separator: " ").contains(where: { normalize(String($0)).hasPrefix(key) && !$0.allSatisfy(\.isNumber) }) {
                prefix.append(book)
            }
        }
        return exact + prefix
    ...
    static func normalize(_ text: String) -> String {
        text.lowercased().filter { $0 != "." && !$0.isWhitespace }
    }
}
