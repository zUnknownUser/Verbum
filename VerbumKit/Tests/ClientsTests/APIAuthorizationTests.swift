...
@Suite struct APIAuthorizationTests {
    ...
    @Test func paidJSONAndAudioAttachIdentityButPublicContentDoesNot() async throws {
        ...
        let tokens = Tokens()
        let api = VerbumAPI(baseURL: URL(string: "https://api.example.test")!, transport: script.transport, cache: .inMemory, tokenProvider: { await tokens.token($0) }, appCheckProvider: { "verified-app-token" })
        ...
        #expect(script.requests[0].value(forHTTPHeaderField: "X-Firebase-AppCheck") == nil)
        #expect(script.requests[1].value(forHTTPHeaderField: "X-Firebase-AppCheck") == "verified-app-token")
        #expect(script.requests[2].value(forHTTPHeaderField: "X-Firebase-AppCheck") == "verified-app-token")
    ...
    @Test func searchUsesExistingIdentityWithoutCreatingGuestAndSeparatesCache() async throws {
        ...
        let tokens = Tokens()
        let signedAPI = VerbumAPI(baseURL: base, transport: script.transport, cache: cache, tokenProvider: { await tokens.token($0) }, appCheckProvider: { "verified-app-token" })
