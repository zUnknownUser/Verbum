...
const sourceJSON = `jsonb_build_object('id',s.id,'citation',s.citation,'url',s.url)`
...
func (s *Store) Entity(ctx context.Context, id string) (domain.Entity, error) {
	return readJSON[domain.Entity](ctx, s, `SELECT `+entityJSON+` FROM entities e WHERE e.id=$1`, id)
}

func (s *Store) Entities(ctx context.Context, kind domain.EntityType) ([]domain.Entity, error) {
	return readJSON[[]domain.Entity](ctx, s, `SELECT COALESCE(jsonb_agg(`+entityJSON+` ORDER BY e.name COLLATE "C",e.position,e.id),'[]'::jsonb)
 FROM entities e WHERE e.type=$1 AND e.type<>'passage'`, kind)
}

func (s *Store) Search(ctx context.Context, query string) ([]domain.Entity, error) {
	q := strings.ToLower(strings.TrimSpace(query))
	return readJSON[[]domain.Entity](ctx, s, `SELECT COALESCE(jsonb_agg(`+entityJSON+` ORDER BY e.position,e.id),'[]'::jsonb)
 FROM entities e WHERE e.type<>'passage' AND $1<>'' AND strpos(lower(e.name),$1)>0`, q)
}
...
// SearchPassages blends lexical (full-text) and semantic (pgvector cosine) retrieval over
// scripture_verses: each channel proposes its own top-limit candidates, then the union is
// reranked by the sum of both channels' scores (0 for a channel that didn't propose a row).
...
func (s *Store) SearchPassages(ctx context.Context, queryText string, queryEmbedding []float32, limit int) ([]domain.PassageReference, error) {
	...
	return readJSON[[]domain.PassageReference](ctx, s, `WITH lexical AS (
...
// EntitiesForPassages returns the ids of entities whose curated keyPassages (entity_key_passages)
...
func (s *Store) EntitiesForPassages(ctx context.Context, refs []domain.PassageReference) ([]string, error) {
	...
	rows, err := s.pool.Query(ctx, `SELECT DISTINCT p.entity_id FROM entity_key_passages p
...
func (s *Store) Detail(ctx context.Context, id string) (domain.EntityDetail, error) {
 ...
 'entity',`+entityJSON+`,
 'aliases',COALESCE((SELECT jsonb_agg(a.alias ORDER BY a.position,a.alias) FROM entity_aliases a WHERE a.entity_id=e.id),'[]'::jsonb),
 'approximateDates',d.approximate_dates,'role',d.role,'modernGeography',d.modern_geography,
 ...
 'sources',CASE WHEN d.entity_id IS NULL THEN
 COALESCE((SELECT jsonb_agg(`+sourceJSON+` ORDER BY es.position,s.id) FROM entity_sources es JOIN sources s ON s.id=es.source_id WHERE es.entity_id=e.id),
 (SELECT jsonb_agg(`+sourceJSON+`) FROM sources s WHERE s.id='fixture.source.editorial'),'[]'::jsonb)
 ELSE COALESCE((SELECT jsonb_agg(`+sourceJSON+` ORDER BY ds.position,s.id) FROM entity_detail_sources ds JOIN sources s ON s.id=ds.source_id WHERE ds.entity_id=e.id),'[]'::jsonb) END)
 FROM entities e LEFT JOIN entity_details d ON d.entity_id=e.id WHERE e.id=$1`, id)
}
...
func (s *Store) Neighbors(ctx context.Context, id string, limit int) (domain.GraphSnapshot, error) {
 ...
 SELECT jsonb_build_object('root',`+entityJSON+`,
 'nodes',COALESCE((SELECT jsonb_agg(`+entityJSON+` ORDER BY e.confidence DESC,e.name COLLATE "C",e.edge_position,e.id) FROM neighbors e),'[]'::jsonb),
 ...
 FROM entities e WHERE e.id=$1`, id, limit)
}
...
func (s *Store) Timeline(ctx context.Context, entityID string) (domain.Timeline, error) {
 ...
 SELECT t.* FROM timeline_events t WHERE $1='' OR EXISTS (SELECT 1 FROM timeline_event_entities x WHERE x.event_id=t.id AND x.entity_id=$1)
 ) SELECT jsonb_build_object('events',COALESCE((SELECT jsonb_agg(jsonb_build_object(
 'id',t.id,'title',t.title,'startYear',t.start_year,'endYear',t.end_year,'datePrecision',t.date_precision,'summary',t.summary,
 ...
 'entityNames',COALESCE((SELECT jsonb_object_agg(e.id,e.name) FROM entities e WHERE EXISTS
 (SELECT 1 FROM timeline_event_entities x JOIN events t ON t.id=x.event_id WHERE x.entity_id=e.id)),'{}'::jsonb))`, entityID)
}
