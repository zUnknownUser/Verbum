...
func (s *Store) EntitySources(ctx context.Context, ids []string) ([]domain.SourceReference, error) {
	return readJSON[[]domain.SourceReference](ctx, s, `SELECT COALESCE(jsonb_agg(`+sourceJSON+` ORDER BY s.id),'[]'::jsonb)
 FROM sources s WHERE s.id IN (
 SELECT r.source_id FROM entity_source_records r WHERE r.entity_id=ANY($1::text[]))`, ids)
}
